...
const getIssuesFromRepos = async function (access_token) {
  ...
  const repoFetch = await fetch(
    `https://api.github.com/orgs/ChicoState/repos?per_page=5&page=1`,
    {
      method: "get",
      headers: {
        "Content-Type": "application/json",
  ...
  /* const data = await fetch(
    ...
    .then((json) => {
      ...
      arr.forEach((data) => {
        ...
        console.log("Data number >" + data.number + " rep => " + repo);
...
*/
  ...
  const resultArr = await Promise.all(
    repoData.map((res) => {
      ...
      console.log(" name of repo => " + res);
      

     // repoName = repoName.substring(repoName.lastIndexOf("/"), repoName.length() - 1);
     // console.log("Repo NAME => " + repoName);
      return fetch(
        `https://api.github.com/repos/${repo}/issues/${res.number}`,
        {
          method: "get",
          headers: {
            "Content-Type": "application/json",
  ...
  console.log("reslt arr name => " + resultArr[0].repoName);
  return resultArr;
};
